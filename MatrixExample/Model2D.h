...
private:
	...
	double phi; // Угол поворота модели относительно исходного положения
...
public:
	...
	void Apply(Matrix<>);
	void ApplyWithSupportPoint(); // подстановка новых зависящих от опорной точки параметров, описвающих положение модели

	void SupportPointRotation(double); // Вращение относительно опорной точки
	void SupportPointTranslation(double, double); // Перенос опорной точки модели на вектор
	void SupportPointScaling(double, double); // Масштабирование модели относительно горизонтальной и вертикальной локальных осей
